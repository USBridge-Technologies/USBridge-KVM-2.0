...
The **Virtual Media** subsystem enables the remote, out-of-band attachment of standard optical disc images (`.iso`) directly to the target server. By emulating a physical USB CD/DVD-ROM drive at the hardware level, this architecture provides seamless pre-OS media access without requiring a complex, dependency-heavy PXE network boot infrastructure.
...
* **Bare-Metal ISO Mounting:** Attach bootable installer disk images directly to execute zero-touch operating system installations or deploy low-level, isolated diagnostic utilities.
...
* **Universal Hardware Emulation:** The target host's USB controller recognizes the attached virtual media as a standard, physically connected USB optical drive. This ensures universal plug-and-play compatibility across both legacy text-based BIOS and modern UEFI environments without requiring any custom host-level drivers.

---

## 2. Media Provisioning & Staging Workflow

From the perspective of the target server, the virtual mounting process is completely transparent and perfectly mimics the physical insertion of an external optical drive. 
...
3. **Host Optimization:** The target server's hardware subsystem immediately enumerates a new USB optical device line. This makes the bootable media instantly available within the native BIOS/UEFI Boot Menu for direct OS deployment.
