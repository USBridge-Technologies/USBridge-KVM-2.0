...
| Function | Description |
| :--- | :--- |
| `insert_media(source, drive_mode="")` | Attaches one more disk/ISO source on top of whatever is already mounted. `source` follows the same resolution rules as the mount API's `sources[]` (bare filename searched under `/mnt/sdcard/iso` / `/mnt/emmc/iso`, or a full path/URL). |
