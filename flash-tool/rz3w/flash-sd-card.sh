...
# card reader -- no USB/rkdeveloptool/Maskrom involved. This is the
# alternative to flash-device-fast.sh when you want to run the appliance
# straight off an SD card instead of (re)flashing the onboard eMMC: insert
# the card into a reader on your computer, write it here, then put it in
# the appliance's SD slot and power on.
#
# idbloader/U-Boot-proper/ATF are baked into the .gptimg at fixed LBA
# offsets, and the RK3566 boot ROM reads those same offsets from whichever
# medium it boots off (SD or eMMC) -- so a straight raw copy of the image
# onto the card carries the whole boot chain, same as an eMMC flash.
#
# On a board with a real eMMC also present, the firmware dynamically
# detects the actual boot device at runtime (U-Boot env/bootcount and the
# kernel's /uboot + /data mounts all follow whichever medium actually
# booted), and the on-device Settings menu offers "Install to eMMC" once
# booted this way -- copies the running system onto the eMMC and powers
# off, so you can move from SD to eMMC without re-flashing from a computer
...
echo "  • This card boots and runs standalone -- same partition layout as an"
echo "    eMMC flash (idbloader/U-Boot/ATF are baked into the gptimg at fixed"
echo "    offsets that the RK3566 boot ROM reads the same way from either medium)."
echo "  • Works correctly even with an eMMC also present on the board: the"
echo "    firmware dynamically detects the actual boot device for both the"
echo "    U-Boot env/bootcount and the /uboot + /data mounts."
echo "  • On a board with a real eMMC, the on-device Settings menu offers"
echo "    \"Install to eMMC\" once booted from this card -- copies the running"
echo "    system onto the eMMC and powers off; remove the SD card afterward"
echo "    and power back on to boot from eMMC."
echo ""
